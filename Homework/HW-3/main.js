...
let a = 0;
while (a < 20) {
    document.write(`<h1>random text - # ${a}</h1>`)
    a++;
}
...
for (const product of products) {
    document.write(`<div class="product-card">`)
        ...
        document.write(`<img style="width: 100px" src="${product.image}" alt="" class="product-image">`)
    document.write(`</div class="product-card">`)
}
...
for (const user of users) {
    if (user.status){
        document.write(`<p>користувач зі статусом true - ${user.name}</p>`)
    }
}

for (const user of users) {
    if (!user.status){
        document.write(`<p>користувач зі статусом true - ${user.false}</p>`)
    }
}

for (const user of users) {
    if (user.age > 30){
        document.write(`<p>користувач старше 30 років - ${user.name}</p>`)
    }
}